...
if __name__ == '__main__':
    pred_file = 'results/poster/MDMTL/stage2/poster_mdmtl_40_cw/predictions_av/checkpoint_valid_combined_score/test_ravdess.json'
    y_pred_ec, y_true_ec, _ = load_ec_predictions(pred_file)
    metrics_ec = calculate_emotion_class(y_pred_ec, y_true_ec, None, n_classes=7)
    y_pred_ei, y_true_ei, _ = load_ei_predictions(pred_file)
    metrics_ei = calculate_emotion_class(y_pred_ei, y_true_ei, None, n_classes=3)
    print('[AV] EC F1:', metrics_ec['F1'])
    print('[AV] EI F1:', metrics_ei['F1'])


    pred_file = 'results/poster/MDMTL/stage2/poster_mdmtl_40_cw/predictions_a/checkpoint_valid_combined_score/test_ravdess.json'
    y_pred_ec, y_true_ec, _ = load_ec_predictions(pred_file)
    metrics_ec = calculate_emotion_class(y_pred_ec, y_true_ec, None, n_classes=7)
    y_pred_ei, y_true_ei, _ = load_ei_predictions(pred_file)
    metrics_ei = calculate_emotion_class(y_pred_ei, y_true_ei, None, n_classes=3)
    print('[A] EC F1:', metrics_ec['F1'])
    print('[A] EI F1:', metrics_ei['F1'])
    

    pred_file = 'results/poster/MDMTL/stage2/poster_mdmtl_40_cw/predictions_v/checkpoint_valid_combined_score/test_ravdess.json'
    y_pred_ec, y_true_ec, _ = load_ec_predictions(pred_file)
    metrics_ec = calculate_emotion_class(y_pred_ec, y_true_ec, None, n_classes=7)
    y_pred_ei, y_true_ei, _ = load_ei_predictions(pred_file)
    metrics_ei = calculate_emotion_class(y_pred_ei, y_true_ei, None, n_classes=3)
    print('[V] EC F1:', metrics_ec['F1'])
    print('[V] EI F1:', metrics_ei['F1'])
