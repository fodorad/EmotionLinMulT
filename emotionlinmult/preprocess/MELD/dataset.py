...
from emotionlinmult.preprocess import MELD_EMOTION_MAPPING
...
def _process_sample(sample):
    ...
    sample['emotion_class.npy'] = torch.tensor(MELD_EMOTION_MAPPING.to_unified_id(sample['emotion_class.npy'].item()), dtype=torch.int64)
    sample['sentiment_class.npy'] = torch.tensor(sample['sentiment_class.npy'].item(), dtype=torch.int64)
...
def calculate_class_distribution():
    """Calculate and return class distribution statistics for MELD dataset.
    ...
    for subset in ['train', 'valid', 'test']:
        ...
        for sample in tqdm(dataset, desc=f"Processing {subset} samples"):
            ...
            # Get unified names
            unified_emotion_name = MELD_EMOTION_MAPPING.unified_name(unified_class_id)

            # Get original dataset class and intensity (reverse mapping from unified to original)
            original_class_id = MELD_EMOTION_MAPPING.to_orig_id(unified_class_id)
            original_class_name = MELD_EMOTION_MAPPING.to_orig_name(original_class_id)
...
if __name__ == "__main__":
    ...
    #print("Class counts:", train_counts) # tensor([4797, 1799, 1223,  705, 1128,  276,  281,    0])
    #print("Class weights:", class_weights) # tensor([0.1318, 0.3515, 0.5171, 0.8970, 0.5606, 2.2913, 2.2506, 0.0000])
    #exit()
