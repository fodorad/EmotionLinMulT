...
from emotionlinmult.preprocess import CELEBVHQ_EMOTION_MAPPING
from emotionlinmult.preprocess.CelebVHQ import DB_PROCESSED


def _process_sample(sample):

    for key in ["emotion_class.npy", "appearance.npy", "action.npy", "clip.npy", "wavlm_baseplus.npy"]:
        if key in sample:
            sample[key] = torch.from_numpy(sample[key].copy())

    # convert emotion class to unified emotion class
    sample['emotion_class.npy'] = torch.tensor(CELEBVHQ_EMOTION_MAPPING.to_unified_id(sample['emotion_class.npy'].item()), dtype=torch.int64)
...
def get_shard_urls(subset: str):
    ...
    pattern = f"celebvhq_{subset}_*.tar"
...
def calculate_class_distribution():
    """Calculate and return class distribution statistics for CelebV-HQ dataset.
    ...
    for subset in ['train', 'valid', 'test']:
        ...
        for sample in tqdm(dataset, desc=f"Processing {subset} samples"):
            ...
            # Get unified names
            unified_emotion_name = CELEBVHQ_EMOTION_MAPPING.unified_name(unified_class_id)

            # Get original dataset class and intensity (reverse mapping from unified to original)
            original_class_id = CELEBVHQ_EMOTION_MAPPING.to_orig_id(unified_class_id)
            original_class_name = CELEBVHQ_EMOTION_MAPPING.to_orig_name(original_class_id)
...
def compute_class_weights():
    ...
    weights = 1.0 / (train_counts + 1e-6)  # add small epsilon to avoid division by zero
    # Normalize weights
...
if __name__ == "__main__":
    ...
    #print("Class weights:", class_weights) # Class weights: tensor([0.0218, 0.0937, 1.0599, 0.2366, 0.3231, 1.5054, 3.0672, 1.6922])

    test_dataset = create_dataset("test")
    for i, sample in tqdm(enumerate(test_dataset), desc="Loading test samples"):
        print(list(sample.keys()))
        print(sample['__key__'], sample['emotion_class.npy'].item(), UNIFIED_TO_EMOTION_NAMES[sample['emotion_class.npy'].item()])
        if i == 10: break
